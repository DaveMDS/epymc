...
_volume: float = 0.0  # Linear volume between 0 and MAX (100 by default)
_volume_muted: bool = False
_player: Optional['EmcPlayerBase'] = None  # EmcVideoPlayer or EmcAudioPlayer instance, or None
_saved_player: Optional['EmcPlayerBase'] = None  # EmcAudioPlayer while EmcVideoPlayer is active
_onair_url: Optional[str] = None
_onair_title: Optional[str] = None
_onair_poster: Optional[str] = None
_play_db: Optional[EmcDatabase] = None  # key: url  data: {'started': 14, 'finished': 0, 'stop_at': 0 }
...
def stop(emit_playback_finished=False):
    ...
    if isinstance(_player, EmcVideoPlayer):
        ...
        if _player.position >= _player.play_length - 5 or _player.position == 0.0:
            # NOTE: vlc set the pos at zero when finished :/
            counts['finished'] += 1
            counts['stop_at'] = 0
        else:
...
class EmcPlayerBase(object):
    ...
    def position_percent(self):
        ...
        pos, length = self.position, self.play_length
        return (pos / length) if length > 0 else 0.0
...
class EmcAudioPlayer(gui.EmcLayout, EmcPlayerBase):
    def __init__(self, url=None):
        ...
        self.box_append('buttons.box', EmcButton(self, icon='icon/prev',
                                                 cb=lambda _: input_events.event_emit('PLAYLIST_PREV')))
        self.box_append('buttons.box', EmcButton(self, icon='icon/pause',
                                                 cb=lambda _: input_events.event_emit('TOGGLE_PAUSE'),
                                                 name='PlayPauseBtn'))
        self.box_append('buttons.box', EmcButton(self, icon='icon/next',
                                                 cb=lambda _: input_events.event_emit('PLAYLIST_NEXT')))
        self.box_append('buttons.box', EmcButton(self, icon='icon/stop',
                                                 cb=lambda _: input_events.event_emit('STOP')))
    ...
    # genlist item class
    @staticmethod
    def _gl_text_get(obj, part, pl_item):
    ...
    @staticmethod
    def _genlist_item_activated_cb(gl, it):
    ...
    def focus_move_internal(self, direction):
        if self.focus_move(direction) is False:
...
class EmcVideoPlayer(gui.EmcLayout, EmcPlayerBase):
    ...
    def __init__(self, url=None):
        ...
        # ### set title + poster
        if _onair_title:
            self.title_set(_onair_title)
        if _onair_poster:
            self.poster_set(_onair_poster)

    def delete(self):
        input_events.listener_del('EmcVideoPlayer')
        events.listener_del('EmcVideoPlayer')
        if self._update_timer:
            self._update_timer.delete()
        if self._buffer_dialog:
            self._buffer_dialog.delete()
        if self._subtitles:
            self._subtitles.delete()
        if self._subs_timer:
            self._subs_timer.delete()
        if self._subs_notify:
            self._subs_notify.delete()
