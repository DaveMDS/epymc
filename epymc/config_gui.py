...
from epymc import gui, mainmenu, ini, modules, utils
...
# private globals
_browser: EmcBrowser  # EmcBrowser instance
...
class StdConfigItemBase(object):
    ...
    @staticmethod
    def poster_get(url, user_data):
        return None

    @staticmethod
    def cover_get(url, user_data):
        return None

    @staticmethod
    def fanart_get(url, user_data):
        return None
...
class StdConfigItemLang(StdConfigItemBase):
    ...
    def _dia_list_selected_cb(self, dia):
        ...
        if self._mul:
            li = ini.get_string_list(self._sec, self._opt)
            li.remove(lang) if lang in li else li.append(lang)
            if len(li) < 1:
                li.append('en')
            ini.set_string_list(self._sec, self._opt, li)
        else:
...
class StdConfigItemNumber(StdConfigItemBase):
    def __init__(self, fmt, udm, vmin, vmax, step, *args):
        self._fmt = fmt
        self._udm = udm
        self._min = vmin
        self._max = vmax
...
def standard_item_number_add(section, option, label, icon=None, info=None, cb=None,
                             fmt='%.0f', udm='', vmin=0.0, vmax=100.0, step=1.0):
    """ TODOC """
    _browser.item_add(StdConfigItemNumber(fmt, udm, vmin, vmax, step, section, option, label, icon, info, cb),
                      'config://%s/%s' % (section, option), None)
...
def _general_populate(browser, url):
    ...
    standard_item_number_add('general', 'scale', _('Interface scale'), 'icon/scale', cb=_change_scale, fmt='%.1f',
                             udm='x', vmin=0.5, vmax=2.0, step=0.1)
    ...
    standard_item_number_add('general', 'max_concurrent_download', _('Max concurrent download'), 'icon/download',
                             fmt='%.0f', vmin=1, vmax=10, step=1)
    ...
    li = ['vlc', 'gstreamer1', 'gstreamer', 'xine', 'generic']
    standard_item_string_from_list_add('mediaplayer', 'backend',
                                       _('Multimedia engine'), li, 'icon/evas')

    standard_item_number_add('general', 'fps', _('Frames per second'), 'icon/evas', cb=_change_fps, fmt='%.0f',
                             udm='fps', vmin=10, vmax=120, step=10)
    standard_item_number_add('thumbnailer', 'max_concurrent_thumb', _('Max concurrent thumbnails'), cb=_change_thumbs,
                             fmt='%.0f', vmin=1, vmax=10, step=1)
    standard_item_number_add('thumbnailer', 'max_generation_time', _('Max thumbnails generation time'),
                             cb=_change_thumbs, fmt='%.0f', udm=_('seconds'), vmin=5, vmax=60, step=5)
...
def _audio_populate(browser, url):
    standard_item_number_add('mediaplayer', 'volume_adjust_step', _('Volume adjustment step'), 'icon/volume',
                             fmt='%.0f', udm='%', vmin=1, vmax=15, step=1)
    standard_item_number_add('mediaplayer', 'volume_maximum', _('Volume maximum value'), 'icon/volume', fmt='%.0f',
                             udm='%', vmin=50, vmax=200, step=10)
...
def _views_populate(browser, url):
    ...
    standard_item_number_add('general', 'view_postergrid_size', _('Poster grid items size'), 'icon/view_postergrid',
                             fmt='%.0f', udm='px', vmin=50, vmax=500, step=25)
    standard_item_number_add('general', 'view_covergrid_size', _('Cover grid items size'), 'icon/view_covergrid',
                             fmt='%.0f', udm='px', vmin=50, vmax=500, step=25)
