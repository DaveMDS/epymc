...
_udev_module: 'EmcDeviceManagerUdev'  # EmcDeviceManagerUdev instance
...
def partition_hum_size(bites):
    """ Get the human readable size like reported by UDisk2 """
    bites = float(bites)
    if bites > 1000000000000:
        size = bites / 1000000000000
        unit = 'TB'
    elif bites > 1000000000:
        size = bites / 1000000000
        unit = 'GB'
    elif bites > 1000000:
        size = bites / 1000000
        unit = 'MB'
...
class EmcDeviceManagerUdev:
    ...
    def udev_device_filter(self, action, udevice, thread=True):
        ...
        if udevice.device_type == 'disk':
            ...
            if udevice.get('ID_CDROM_MEDIA_CD') == '1' and udevice.get('ID_CDROM_MEDIA_TRACK_COUNT_AUDIO') is not None:
