...
def get_available_themes():
    ...
    li = glob.glob(os.path.join(user_conf_dir, 'themes', '*.edj'))

    # then search inside the package
    li += glob.glob(os.path.join(emc_base_dir, 'themes', '*.edj'))
...
def hum_size(bites):
    ...
    if bites >= 1099511627776:
        terabytes = bites / 1099511627776
        size = '%.3fT' % terabytes
    elif bites >= 1073741824:
        gigabytes = bites / 1073741824
        size = '%.2fG' % gigabytes
    elif bites >= 1048576:
        megabytes = bites / 1048576
        size = '%.1fM' % megabytes
    elif bites >= 1024:
        kilobytes = bites / 1024
        size = '%.0fK' % kilobytes
    else:
        size = '%.0fb' % bites
    return size
...
def http_error_code_to_str(code):
    import http.server
    try:
        return http.server.BaseHTTPRequestHandler.responses[code][0]
    except IndexError:
        return _('Unknown')
